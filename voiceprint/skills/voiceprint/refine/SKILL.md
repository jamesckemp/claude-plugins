...
```
Phase 1: Load & Ask          (~30s)  → Read file silently, ask what to refine
...
**Old format detection:** If both `SKILL.md` and `voice-profile.md` exist in the directory, and SKILL.md does NOT have a `voiceprint-version` field in its frontmatter, the skill was generated by an older voiceprint version that used two files. Tell the user:

> This writer skill uses the older two-file format. Before refining, run `/voiceprint update {TARGET_DIR}` to merge it into the new single-file format. Then come back here to refine.
...
- **Old two-file format**: Detect `voice-profile.md` without `voiceprint-version` in SKILL.md frontmatter. Direct user to `/voiceprint update` first.
