...
### Step 1: Read and check format (GATE)
...
**Format version gate — check both of these before continuing:**

1. Check the SKILL.md frontmatter for a `voiceprint-version` field.
2. Check whether `voice-profile.md` exists in the same directory.

> **CRITICAL — old two-file format detected:**
> If `voice-profile.md` exists in the directory AND `voiceprint-version` is missing from the SKILL.md frontmatter, this skill was generated by an older voiceprint version.

**Stop. Do not continue with the refine workflow.**

Tell the user:

> This writer skill uses the older two-file format. Run `/voiceprint update {TARGET_DIR}` first to merge it into the new single-file format, then come back here to refine.
...
- **Old two-file format**: Caught by the version gate in Phase 1 Step 1. If `voice-profile.md` exists and `voiceprint-version` is missing from frontmatter, hard stop and direct user to `/voiceprint update` first.
